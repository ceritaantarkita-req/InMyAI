...
| 2026-08-11 | SUCCESS | Closed the remaining launcher retention gap: the Node wrapper now removes the lifecycle token from its own environment before spawn, passes it through a one-use child environment, and clears that temporary copy immediately after launch. |
